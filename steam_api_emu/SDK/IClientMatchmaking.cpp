#include "IClientMatchmaking.h"
	...
	virtual void  AddRequestLobbyListDistanceFilter ( ELobbyDistanceFilter filter )	{
		char log[1024];
		sprintf(log,"AddRequestLobbyListDistanceFilter() filter=0x%X ",
			filter		);
		OutputDebugStringA(log);
		filter);
		OutputDebugStringA("\n");
	...
	virtual void  AddRequestLobbyListCompatibleMembersFilter ( CSteamID steamID )	{
		char log[1024];
		sprintf(log,"AddRequestLobbyListCompatibleMembersFilter() steamID=0x%X ",
			steamID		);
		OutputDebugStringA(log);
		steamID);
		OutputDebugStringA("\n");
	...
	virtual void  SetLobbyVoiceEnabled ( CSteamID steamIDLobby,  bool bVoiceEnabled )	{
		char log[1024];
		sprintf(log,"SetLobbyVoiceEnabled() steamIDLobby=0x%X bVoiceEnabled=0x%X ",
			steamIDLobby,
			bVoiceEnabled		);
		OutputDebugStringA(log);
		steamIDLobby, bVoiceEnabled);
		OutputDebugStringA("\n");
	}
	virtual bool  RequestFriendsLobbies ()	{
		char log[1024];
		sprintf(log,"RequestFriendsLobbies() "		);
		OutputDebugStringA(log);
		bool  ret = m_pWrapper->RequestFriendsLobbies();
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	...
	virtual bool  SetLinkedLobby ( CSteamID steamIDLobby,  CSteamID steamIDLobby2 )	{
		char log[1024];
		sprintf(log,"SetLinkedLobby() steamIDLobby=0x%X steamIDLobby2=0x%X ",
			steamIDLobby,
			steamIDLobby2		);
		OutputDebugStringA(log);
		bool  ret = m_pWrapper->SetLinkedLobby(steamIDLobby, steamIDLobby2);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	}
	virtual int  GetGMSServerCount ()	{
		char log[1024];
		sprintf(log,"GetGMSServerCount() "		);
		OutputDebugStringA(log);
		int  ret = m_pWrapper->GetGMSServerCount();
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	}
	virtual bool  GetGMSServerAddress ( int iServer,  uint32 *unServerIP,  uint16 *usServerPort )	{
		char log[1024];
		sprintf(log,"GetGMSServerAddress() iServer=0x%X unServerIP=0x%X usServerPort=0x%X ",
			iServer,
			unServerIP,
			usServerPort		);
		OutputDebugStringA(log);
		bool  ret = m_pWrapper->GetGMSServerAddress(iServer, unServerIP, usServerPort);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	}
	virtual SteamAPICall_t  BeginGMSQuery ( AppId_t nAppId,  int32 iRegionCode,  const char* szFilterText )	{
		char log[1024];
		sprintf(log,"BeginGMSQuery() nAppId=0x%X iRegionCode=0x%X szFilterText=%s ",
			nAppId,
			iRegionCode,
			szFilterText		);
		OutputDebugStringA(log);
		SteamAPICall_t  ret = m_pWrapper->BeginGMSQuery(nAppId, iRegionCode, szFilterText);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	}
	virtual int32  PollGMSQuery ( SteamAPICall_t hGMSQuery )	{
		char log[1024];
		sprintf(log,"PollGMSQuery() hGMSQuery=0x%X ",
			hGMSQuery		);
		OutputDebugStringA(log);
		int32  ret = m_pWrapper->PollGMSQuery(hGMSQuery);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	}
	virtual int32  GetGMSQueryResults ( SteamAPICall_t hGMSQuery,  GMSQueryResult_t *pGMSQueryResult,  int32 iResultIndex )	{
		char log[1024];
		sprintf(log,"GetGMSQueryResults() hGMSQuery=0x%X pGMSQueryResult=0x%X iResultIndex=0x%X ",
			hGMSQuery,
			pGMSQueryResult,
			iResultIndex		);
		OutputDebugStringA(log);
		int32  ret = m_pWrapper->GetGMSQueryResults(hGMSQuery, pGMSQueryResult, iResultIndex);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	}
	virtual void  ReleaseGMSQuery ( SteamAPICall_t hGMSQuery )	{
		char log[1024];
		sprintf(log,"ReleaseGMSQuery() hGMSQuery=0x%X ",
			hGMSQuery		);
		OutputDebugStringA(log);
		hGMSQuery);
		OutputDebugStringA("\n");
