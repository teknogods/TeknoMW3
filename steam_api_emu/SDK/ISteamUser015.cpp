...
public:
	...
	virtual EVoiceResult  DecompressVoice ( void *pCompressed,  uint32 cbCompressed,  void *pDestBuffer,  uint32 cbDestBufferSize,  uint32 *nBytesWritten,  uint32 nSamplesPerSec )	{
		char log[1024];
		sprintf(log,"DecompressVoice() pCompressed=0x%X cbCompressed=0x%X pDestBuffer=0x%X cbDestBufferSize=0x%X nBytesWritten=0x%X nSamplesPerSec=0x%X ",
			pCompressed,
			cbCompressed,
			pDestBuffer,
			cbDestBufferSize,
			...
			nSamplesPerSec		);
		OutputDebugStringA(log);
		EVoiceResult  ret = m_pWrapper->DecompressVoice(pCompressed, cbCompressed, pDestBuffer, cbDestBufferSize, nBytesWritten, nSamplesPerSec);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
