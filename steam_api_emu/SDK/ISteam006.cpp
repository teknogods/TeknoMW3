...
class ISteam006_wrapper
{
public:
	virtual ~ISteam006_wrapper();
	...
	virtual SteamHandle_t  OpenFileEx (const char* cszFileName,  const char* cszMode,  int iArg3,  unsigned int* puSize,  int* piArg5,  TSteamError* pError)	{
		char log[1024];
		sprintf(log,"OpenFileEx() cszFileName=%s cszMode=%s iArg3=0x%X puSize=0x%X piArg5=0x%X pError=0x%X ",
			cszFileName,
			cszMode,
			iArg3,
			puSize,
			...
			pError		);
		OutputDebugStringA(log);
		SteamHandle_t  ret = m_pWrapper->OpenFileEx(cszFileName, cszMode, iArg3, puSize, piArg5, pError);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	...
	virtual int  FindServersNumServers (ESteamServerType eSteamServerType)	{
		char log[1024];
		sprintf(log,"FindServersNumServers() eSteamServerType=0x%X ",
			eSteamServerType		);
		OutputDebugStringA(log);
		int  ret = m_pWrapper->FindServersNumServers(eSteamServerType);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	...
	virtual int  FindServersIterateServer (ESteamServerType eSteamServerType,  unsigned int uIndex,  char* szServerAddress,  int iServerAddressChars)	{
		char log[1024];
		sprintf(log,"FindServersIterateServer() eSteamServerType=0x%X uIndex=0x%X szServerAddress=%s iServerAddressChars=0x%X ",
			eSteamServerType,
			uIndex,
			szServerAddress,
			iServerAddressChars		);
		OutputDebugStringA(log);
		int  ret = m_pWrapper->FindServersIterateServer(eSteamServerType, uIndex, szServerAddress, iServerAddressChars);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	}
	virtual const char*  FindServersGetErrorString ()	{
		char log[1024];
		sprintf(log,"FindServersGetErrorString() "		);
		OutputDebugStringA(log);
		const char*  ret = m_pWrapper->FindServersGetErrorString();
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	...
	virtual int  CheckAppOwnership (unsigned int uAppId,  int* pbOwned,  TSteamGlobalUserID* pSteamGlobalUserID,  TSteamError* pError)	{
		char log[1024];
		sprintf(log,"CheckAppOwnership() uAppId=0x%X pbOwned=0x%X pSteamGlobalUserID=0x%X pError=0x%X ",
			uAppId,
			pbOwned,
			pSteamGlobalUserID,
			pError		);
		OutputDebugStringA(log);
		int  ret = m_pWrapper->CheckAppOwnership(uAppId, pbOwned, pSteamGlobalUserID, pError);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	...
	virtual SteamCallHandle_t  GetCachePercentFragmentation (unsigned int uAppId,  unsigned int* puPercentFragmented,  TSteamError* pError)	{
		char log[1024];
		sprintf(log,"GetCachePercentFragmentation() uAppId=0x%X puPercentFragmented=0x%X pError=0x%X ",
			uAppId,
			puPercentFragmented,
			pError		);
		OutputDebugStringA(log);
		SteamCallHandle_t  ret = m_pWrapper->GetCachePercentFragmentation(uAppId, puPercentFragmented, pError);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	...
	virtual SteamCallHandle_t  DefragCaches (unsigned int uAppId,  TSteamError* pError)	{
		char log[1024];
		sprintf(log,"DefragCaches() uAppId=0x%X pError=0x%X ",
			uAppId,
			pError		);
		OutputDebugStringA(log);
		SteamCallHandle_t  ret = m_pWrapper->DefragCaches(uAppId, pError);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	}
	virtual int  IsFileNeededByApp (unsigned int uAppId,  const char* cszFileName,  unsigned int uFileSize,  unsigned int* puCacheId,  TSteamError* pError)	{
		char log[1024];
		sprintf(log,"IsFileNeededByApp() uAppId=0x%X cszFileName=%s uFileSize=0x%X puCacheId=0x%X pError=0x%X ",
			uAppId,
			cszFileName,
			uFileSize,
			puCacheId,
			pError		);
		OutputDebugStringA(log);
		int  ret = m_pWrapper->IsFileNeededByApp(uAppId, cszFileName, uFileSize, puCacheId, pError);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	}
	virtual SteamCallHandle_t  WaitForAppResources (unsigned int uAppId,  const char* cszMasterList,  TSteamError* pError)	{
		char log[1024];
		sprintf(log,"WaitForAppResources() uAppId=0x%X cszMasterList=%s pError=0x%X ",
			uAppId,
			cszMasterList,
			pError		);
		OutputDebugStringA(log);
		SteamCallHandle_t  ret = m_pWrapper->WaitForAppResources(uAppId, cszMasterList, pError);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	...
	virtual int  ForceCellId (unsigned int uCellId,  TSteamError* pError)	{
		char log[1024];
		sprintf(log,"ForceCellId() uCellId=0x%X pError=0x%X ",
			uCellId,
			pError		);
		OutputDebugStringA(log);
		int  ret = m_pWrapper->ForceCellId(uCellId, pError);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	}
	virtual int  GetAppDLCStatus ( unsigned int nAppID,  unsigned int nDlcId,  int* pbOwned,  TSteamError* pError )	{
		char log[1024];
		sprintf(log,"GetAppDLCStatus() nAppID=0x%X nDlcId=0x%X pbOwned=0x%X pError=0x%X ",
			nAppID,
			nDlcId,
			pbOwned,
			pError		);
		OutputDebugStringA(log);
		int  ret = m_pWrapper->GetAppDLCStatus(nAppID, nDlcId, pbOwned, pError);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	}
	virtual int  StartEngineEx ( TSteamError* pError,  bool bOffline,  bool bRefreshServerCache )	{
		char log[1024];
		sprintf(log,"StartEngineEx() pError=0x%X bOffline=0x%X bRefreshServerCache=0x%X ",
			pError,
			bOffline,
			bRefreshServerCache		);
		OutputDebugStringA(log);
		int  ret = m_pWrapper->StartEngineEx(pError, bOffline, bRefreshServerCache);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	}
	virtual int  IsUsingSdkContentServer ( int* pbIsUsingSdkContentServer,  TSteamError* pError )	{
		char log[1024];
		sprintf(log,"IsUsingSdkContentServer() pbIsUsingSdkContentServer=0x%X pError=0x%X ",
			pbIsUsingSdkContentServer,
			pError		);
		OutputDebugStringA(log);
		int  ret = m_pWrapper->IsUsingSdkContentServer(pbIsUsingSdkContentServer, pError);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	}
	virtual SteamCallHandle_t  SetUser2 ( const char* cszUser,  TSteamError* pError )	{
		char log[1024];
		sprintf(log,"SetUser2() cszUser=%s pError=0x%X ",
			cszUser,
			pError		);
		OutputDebugStringA(log);
		SteamCallHandle_t  ret = m_pWrapper->SetUser2(cszUser, pError);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
