...
class ISteamNetworking001_wrapper
{
public:
	virtual SNetListenSocket_t  CreateListenSocket ( int nVirtualP2PPort,  uint32 nIP,  uint16 nPort )	{
		char log[1024];
		sprintf(log,"CreateListenSocket() nVirtualP2PPort=0x%X nIP=0x%X nPort=0x%X ",
			nVirtualP2PPort,
			nIP,
			nPort		);
		OutputDebugStringA(log);
		SNetListenSocket_t  ret = m_pWrapper->CreateListenSocket(nVirtualP2PPort, nIP, nPort);
	...
	virtual SNetSocket_t  CreateP2PConnectionSocket ( CSteamID steamIDTarget,  int nVirtualPort,  int nTimeoutSec )	{
		char log[1024];
		sprintf(log,"CreateP2PConnectionSocket() steamIDTarget=0x%X nVirtualPort=0x%X nTimeoutSec=0x%X ",
			steamIDTarget,
			nVirtualPort,
			nTimeoutSec		);
		OutputDebugStringA(log);
		SNetSocket_t  ret = m_pWrapper->CreateP2PConnectionSocket(steamIDTarget, nVirtualPort, nTimeoutSec);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	...
	virtual bool  RetrieveData ( SNetListenSocket_t hListenSocket,  void *pubDest,  uint32 cubDest,  uint32 *pcubMsgSize )	{
		char log[1024];
		sprintf(log,"RetrieveData() hListenSocket=0x%X pubDest=0x%X cubDest=0x%X pcubMsgSize=0x%X ",
			hListenSocket,
			pubDest,
			cubDest,
			pcubMsgSize		);
		OutputDebugStringA(log);
		bool  ret = m_pWrapper->RetrieveData(hListenSocket, pubDest, cubDest, pcubMsgSize);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
