...
class ISteam003_wrapper
{
public:
	virtual ~ISteam003_wrapper();
	...
	virtual SteamCallHandle_t  RefreshAccountInfo (TSteamError* pError)	{
		char log[1024];
		sprintf(log,"RefreshAccountInfo() pError=0x%X ",
			pError		);
		OutputDebugStringA(log);
		SteamCallHandle_t  ret = m_pWrapper->RefreshAccountInfo(pError);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
