...
class ISteamMatchmaking005_wrapper
{
public:
	...
	virtual OBSOLETE_FUNCTION void  AddRequestLobbyListSlotsAvailableFilter ()	{
		char log[1024];
		sprintf(log,"AddRequestLobbyListSlotsAvailableFilter() "		);
		OutputDebugStringA(log);
		);
		OutputDebugStringA("\n");
	}
	...
	virtual OBSOLETE_FUNCTION double  GetLobbyDistance ( CSteamID steamIDLobby )	{
		char log[1024];
		sprintf(log,"GetLobbyDistance() steamIDLobby=0x%X ",
			steamIDLobby		);
		OutputDebugStringA(log);
		OBSOLETE_FUNCTION double  ret = m_pWrapper->GetLobbyDistance(steamIDLobby);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
