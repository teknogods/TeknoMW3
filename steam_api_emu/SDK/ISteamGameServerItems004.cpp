#include "ISteamGameServerItems004.h"
	...
	virtual bool  GetItemAttribute ( ItemID itemid,  uint32 i,  uint32* attribIndex,  float* attribValue)	{
		char log[1024];
		sprintf(log,"GetItemAttribute() itemid=0x%X i=0x%X attribIndex=0x%X attribValue=0x%X ",
			itemid,
			i,
			attribIndex,
			attribValue		);
		OutputDebugStringA(log);
		bool  ret = m_pWrapper->GetItemAttribute(itemid, i, attribIndex, attribValue);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
