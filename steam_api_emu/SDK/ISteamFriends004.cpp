...
class ISteamFriends004_wrapper
{
public:
	...
	virtual CSteamID  GetFriendByIndex ( int iFriend,  EFriendFlags eFriendFlags )	{
		char log[1024];
		sprintf(log,"GetFriendByIndex() iFriend=0x%X eFriendFlags=0x%X ",
			iFriend,
			eFriendFlags		);
		OutputDebugStringA(log);
		CSteamID  ret = m_pWrapper->GetFriendByIndex(iFriend, eFriendFlags);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
