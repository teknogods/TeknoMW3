...
public:
	...
	virtual int  InitiateGameConnection ( void *pBlob,  int cbMaxBlob,  CSteamID steamID,  CGameID gameID,  uint32 unIPServer,  uint16 usPortServer,  bool bSecure,  void *pvSteam2GetEncryptionKey,  int cbSteam2GetEncryptionKey )	{
		char log[1024];
		sprintf(log,"InitiateGameConnection() pBlob=0x%X cbMaxBlob=0x%X steamID=0x%X gameID=0x%X unIPServer=0x%X usPortServer=0x%X bSecure=0x%X pvSteam2GetEncryptionKey=0x%X cbSteam2GetEncryptionKey=0x%X ",
			pBlob,
			cbMaxBlob,
			steamID,
			gameID,
		...
		OutputDebugStringA(log);
		int  ret = m_pWrapper->InitiateGameConnection(pBlob, cbMaxBlob, steamID, gameID, unIPServer, usPortServer, bSecure, pvSteam2GetEncryptionKey, cbSteam2GetEncryptionKey);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
