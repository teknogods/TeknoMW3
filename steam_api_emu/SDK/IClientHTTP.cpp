#include "IClientHTTP.h"
	...
	virtual HTTPRequestHandle  CreateHTTPRequest ( EHTTPMethod eHTTPRequestMethod,  char *pchAbsoluteURL )	{
		char log[1024];
	...
	virtual bool  SetHTTPRequestHeaderValue ( HTTPRequestHandle hRequest,  char *pchHeaderName,  char *pchHeaderValue )	{
		char log[1024];
	...
	virtual bool  SetHTTPRequestGetOrPostParameter ( HTTPRequestHandle hRequest,  char *pchParamName,  char *pchParamValue )	{
		char log[1024];
	...
	virtual bool  CancelHTTPRequest ( HTTPRequestHandle hRequest )	{
		char log[1024];
		sprintf(log,"CancelHTTPRequest() hRequest=0x%X ",
			hRequest		);
		OutputDebugStringA(log);
		bool  ret = m_pWrapper->CancelHTTPRequest(hRequest);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	}
	virtual bool  GetHTTPResponseHeaderSize ( HTTPRequestHandle hRequest,  char *pchHeaderName,  uint32 *unResponseHeaderSize )	{
		char log[1024];
	...
	virtual bool  GetHTTPResponseHeaderValue ( HTTPRequestHandle hRequest,  char *pchHeaderName,  uint8 *pHeaderValueBuffer,  uint32 uBufferSize )	{
		char log[1024];
		sprintf(log,"GetHTTPResponseHeaderValue() hRequest=0x%X pchHeaderName=%s pHeaderValueBuffer=0x%X uBufferSize=0x%X ",
			hRequest,
			pchHeaderName,
			pHeaderValueBuffer,
			uBufferSize		);
		OutputDebugStringA(log);
		bool  ret = m_pWrapper->GetHTTPResponseHeaderValue(hRequest, pchHeaderName, pHeaderValueBuffer, uBufferSize);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	...
	virtual bool  GetHTTPDownloadProgressPct ( HTTPRequestHandle hRequest,  float * pflProgress )	{
		char log[1024];
		sprintf(log,"GetHTTPDownloadProgressPct() hRequest=0x%X pflProgress=0x%X ",
			hRequest,
			pflProgress		);
		OutputDebugStringA(log);
		bool  ret = m_pWrapper->GetHTTPDownloadProgressPct(hRequest, pflProgress);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
