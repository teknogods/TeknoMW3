...
class ISteamMatchmaking001_wrapper
{
public:
	...
	virtual bool  GetFavoriteGame ( int iGame,  uint32 *pnAppID,  uint32 *pnIP,  uint16 *pnConnPort,  uint32 *punFlags,  uint32 *pRTime32LastPlayedOnServer )	{
		char log[1024];
		sprintf(log,"GetFavoriteGame() iGame=0x%X pnAppID=0x%X pnIP=0x%X pnConnPort=0x%X punFlags=0x%X pRTime32LastPlayedOnServer=0x%X ",
			iGame,
			pnAppID,
			pnIP,
			pnConnPort,
		...
		OutputDebugStringA(log);
		bool  ret = m_pWrapper->GetFavoriteGame(iGame, pnAppID, pnIP, pnConnPort, punFlags, pRTime32LastPlayedOnServer);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	}
	virtual int  AddFavoriteGame ( uint32 nAppID,  uint32 nIP,  uint16 nConnPort,  uint32 unFlags,  uint32 rTime32LastPlayedOnServer )	{
		char log[1024];
		sprintf(log,"AddFavoriteGame() nAppID=0x%X nIP=0x%X nConnPort=0x%X unFlags=0x%X rTime32LastPlayedOnServer=0x%X ",
			nAppID,
			nIP,
			nConnPort,
			unFlags,
		...
		OutputDebugStringA(log);
		int  ret = m_pWrapper->AddFavoriteGame(nAppID, nIP, nConnPort, unFlags, rTime32LastPlayedOnServer);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	}
	virtual bool  RemoveFavoriteGame ( uint32 nAppID,  uint32 nIP,  uint16 nConnPort,  uint32 unFlags )	{
		char log[1024];
		sprintf(log,"RemoveFavoriteGame() nAppID=0x%X nIP=0x%X nConnPort=0x%X unFlags=0x%X ",
			nAppID,
			nIP,
			nConnPort,
			unFlags		);
		OutputDebugStringA(log);
		bool  ret = m_pWrapper->RemoveFavoriteGame(nAppID, nIP, nConnPort, unFlags);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	}
	virtual bool  GetFavoriteGame2 ( int iGame,  uint32 *pnAppID,  uint32 *pnIP,  uint16 *pnConnPort,  uint16 *pnQueryPort,  uint32 *punFlags,  uint32 *pRTime32LastPlayedOnServer )	{
		char log[1024];
		sprintf(log,"GetFavoriteGame2() iGame=0x%X pnAppID=0x%X pnIP=0x%X pnConnPort=0x%X pnQueryPort=0x%X punFlags=0x%X pRTime32LastPlayedOnServer=0x%X ",
			iGame,
			pnAppID,
			pnIP,
			pnConnPort,
			...
			pRTime32LastPlayedOnServer		);
		OutputDebugStringA(log);
		bool  ret = m_pWrapper->GetFavoriteGame2(iGame, pnAppID, pnIP, pnConnPort, pnQueryPort, punFlags, pRTime32LastPlayedOnServer);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	}
	virtual int  AddFavoriteGame2 ( uint32 nAppID,  uint32 nIP,  uint16 nConnPort,  uint16 nQueryPort,  uint32 unFlags,  uint32 rTime32LastPlayedOnServer )	{
		char log[1024];
		sprintf(log,"AddFavoriteGame2() nAppID=0x%X nIP=0x%X nConnPort=0x%X nQueryPort=0x%X unFlags=0x%X rTime32LastPlayedOnServer=0x%X ",
			nAppID,
			nIP,
			nConnPort,
			nQueryPort,
			...
			rTime32LastPlayedOnServer		);
		OutputDebugStringA(log);
		int  ret = m_pWrapper->AddFavoriteGame2(nAppID, nIP, nConnPort, nQueryPort, unFlags, rTime32LastPlayedOnServer);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	}
	virtual bool  RemoveFavoriteGame2 ( uint32 nAppID,  uint32 nIP,  uint16 nConnPort,  uint16 nQueryPort,  uint32 unFlags )	{
		char log[1024];
		sprintf(log,"RemoveFavoriteGame2() nAppID=0x%X nIP=0x%X nConnPort=0x%X nQueryPort=0x%X unFlags=0x%X ",
			nAppID,
			nIP,
			nConnPort,
			nQueryPort,
			unFlags		);
		OutputDebugStringA(log);
		bool  ret = m_pWrapper->RemoveFavoriteGame2(nAppID, nIP, nConnPort, nQueryPort, unFlags);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	}
	virtual void  RequestLobbyList ( uint64 ulGameID,  MatchMakingKeyValuePair_t *pFilters,  uint32 nFilters )	{
		char log[1024];
		sprintf(log,"RequestLobbyList() ulGameID=0x%X pFilters=0x%X nFilters=0x%X ",
			ulGameID,
			pFilters,
			nFilters		);
		OutputDebugStringA(log);
		ulGameID, pFilters, nFilters);
		OutputDebugStringA("\n");
	}
	...
	virtual void  CreateLobby ( uint64 ulGameID,  bool bPrivate )	{
		char log[1024];
		sprintf(log,"CreateLobby() ulGameID=0x%X bPrivate=0x%X ",
			ulGameID,
			bPrivate		);
		OutputDebugStringA(log);
		ulGameID, bPrivate);
	...
	virtual void  ChangeLobbyAdmin ( CSteamID steamIDLobby,  CSteamID steamIDNewAdmin )	{
		char log[1024];
		sprintf(log,"ChangeLobbyAdmin() steamIDLobby=0x%X steamIDNewAdmin=0x%X ",
			steamIDLobby,
			steamIDNewAdmin		);
		OutputDebugStringA(log);
		steamIDLobby, steamIDNewAdmin);
		OutputDebugStringA("\n");
	}
