#include "ISteamUserItems002.h"
	...
	virtual SteamAPICall_t  SetItemBlob ( ItemID itemID,  const void *blob,  uint32 size )	{
		char log[1024];
		sprintf(log,"SetItemBlob() itemID=0x%X blob=0x%X size=0x%X ",
			itemID,
			blob,
			size		);
		OutputDebugStringA(log);
		SteamAPICall_t  ret = m_pWrapper->SetItemBlob(itemID, blob, size);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
