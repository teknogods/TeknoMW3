...
class ISteamUserStats005_wrapper
{
public:
	...
	virtual SteamAPICall_t  UploadLeaderboardScore ( SteamLeaderboard_t hSteamLeaderboard,  int32 nScore,  int32 *pScoreDetails,  int cScoreDetailsCount )	{
		char log[1024];
		sprintf(log,"UploadLeaderboardScore() hSteamLeaderboard=0x%X nScore=0x%X pScoreDetails=0x%X cScoreDetailsCount=0x%X ",
			hSteamLeaderboard,
			nScore,
			pScoreDetails,
			cScoreDetailsCount		);
		OutputDebugStringA(log);
		SteamAPICall_t  ret = m_pWrapper->UploadLeaderboardScore(hSteamLeaderboard, nScore, pScoreDetails, cScoreDetailsCount);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
