...
class ISteamNetworking003_wrapper
{
public:
	virtual bool  SendP2PPacket ( CSteamID steamIDRemote,  const void *pubData,  uint32 cubData,  EP2PSend eP2PSendType )	{
		char log[1024];
		sprintf(log,"SendP2PPacket() steamIDRemote=0x%X pubData=0x%X cubData=0x%X eP2PSendType=0x%X ",
			steamIDRemote,
			pubData,
			cubData,
			eP2PSendType		);
		OutputDebugStringA(log);
		bool  ret = m_pWrapper->SendP2PPacket(steamIDRemote, pubData, cubData, eP2PSendType);
	...
	virtual bool  IsP2PPacketAvailable ( uint32 *pcubMsgSize )	{
		char log[1024];
		sprintf(log,"IsP2PPacketAvailable() pcubMsgSize=0x%X ",
			pcubMsgSize		);
		OutputDebugStringA(log);
		bool  ret = m_pWrapper->IsP2PPacketAvailable(pcubMsgSize);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
	...
	virtual bool  ReadP2PPacket ( void *pubDest,  uint32 cubDest,  uint32 *pcubMsgSize,  CSteamID *psteamIDRemote )	{
		char log[1024];
		sprintf(log,"ReadP2PPacket() pubDest=0x%X cubDest=0x%X pcubMsgSize=0x%X psteamIDRemote=0x%X ",
			pubDest,
			cubDest,
			pcubMsgSize,
			psteamIDRemote		);
		OutputDebugStringA(log);
		bool  ret = m_pWrapper->ReadP2PPacket(pubDest, cubDest, pcubMsgSize, psteamIDRemote);
		sprintf(log," returned 0x%X",ret);
		OutputDebugStringA(log);
		OutputDebugStringA("\n");
		return ret;
