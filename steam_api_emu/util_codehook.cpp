...
struct xde_instr
{
	...
	//
	DWORD  src_set;        /* SRC object set (instr. will READ 'em)   */
	DWORD  dst_set;        /* DST object set (instr. will WRITE 'em)  */
	//
	union
	{
		BYTE  addr_b[8];      /* address bytes, size = addrsize          */
		WORD addr_w[4];
		DWORD  addr_d[2];
	...
	union
	{
		BYTE  data_b[8];      /* data (imm) bytes, size = datasize       */
		WORD data_w[4];
		DWORD  data_d[2];
...
{
	BYTE code[5], *addr, *newproc, *tramp, *hookproc, newCode[5];
	struct _HOOK *next;
} HOOK;
...
int /*__inline*/ reg2xset(int reg_sz /*1,2,4*/, int reg)
{
  if (reg_sz == 1)
...
int xde_disasm(/* IN */ BYTE *opcode,
               /* OUT */ struct xde_instr *diza)
{
  BYTE c, *p;
  DWORD flag, a, d, i, xset;
  DWORD mod, reg, rm, index, base;
  ...
  if (*(WORD*)p == 0x0000) flag |= C_BAD;
  if (*(WORD*)p == 0xFFFF) flag |= C_BAD;
...
DWORD makeTramp(void *addr, void *tramp_, char *errormsg, DWORD numBytes, bool jumpBack)
{
	BYTE *tramp = (BYTE *)tramp_;
	BYTE *pos, *trampStart = tramp;

	pos = (BYTE *)addr;

	while(true)
	{
		DWORD done = (DWORD)(pos - addr);
		if (done >= numBytes)
			break;
		DWORD left = numBytes - done;

		if ((*pos == 0xC3) && (left > 1))	//accept RET when its the last needed byte
		{
			#ifdef DEBUGGING_ENABLED
			info("no space left (addr=%08X)", pos);
			#endif
			return 0;
		}

		if ((*pos == 0xC2) && (left > 3))	//accept RET X when its the last needed 2 bytes
		{
			#ifdef DEBUGGING_ENABLED
			info("no space left (addr=%08X)", pos);
			#endif
		...
		if (xde_disasm(pos, &disasm) == 0)
		{
			#ifdef DEBUGGING_ENABLED
			info("disasm error (addr=%08X, opcode=%02X)", pos, *pos);
			#endif
			return 0;
		}

		if (disasm.flag & C_REL)
		{
			DWORD offset;
			...
			switch(*pos)
			{
				case 0x0F:
						switch(*(pos + 1))
						{
							...
							case 0x8E:
							case 0x8F:
									*(WORD *)tramp = *(WORD *)pos; tramp += 2; pos += 2;
									offset = *(DWORD *)pos; pos += 4;
								break;

							default:
									#ifdef DEBUGGING_ENABLED
									info(V("unknown rel opcode (addr=%08X, opcode=0F%02X)"), pos, *(pos + 1));
									#endif
								return FALSE;
				...
				case 0x7F:
						*tramp = 0x0F; tramp += 1;
						*tramp = 0x10 + *pos; tramp += 1; pos += 1;
						offset = *pos; pos += 1;
					break;

				case 0xE8:
						*tramp = 0xE8; tramp += 1; pos += 1;
						offset = *(DWORD *)pos; pos += 4;
					break;

				case 0xEB:
						*tramp = 0xE9; tramp += 1; pos += 1;
						offset = *pos; pos += 1;
					break;
				...
				default:
						#ifdef DEBUGGING_ENABLED
						info(V("unknown rel opcode (addr=%08X, opcode=%02X)"), pos, *(pos + 1));
						#endif
					return FALSE;
			}

			*(DWORD *)tramp = (DWORD)pos + offset - (DWORD)tramp - 4; tramp += 4;
		}
		else
		{
			memcpy(tramp, pos, disasm.len); tramp += disasm.len;
			pos += disasm.len;
		}
	}
	...
	if (jumpBack)
	{
		*(BYTE *)tramp = 0xE9; tramp += 1;
		*(DWORD *)tramp = (DWORD)((DWORD)addr + (pos - (DWORD)addr) - tramp - 4); tramp += 4;
	}

	return tramp - trampStart;
}

bool removeHook(void *addr, const void *oldCode, const void *newCode, DWORD numBytes)
{
	__try
	{
#ifdef KERNEL
...
#ifdef KERNEL
		mdl = IoAllocateMdl(addr, numBytes, FALSE, FALSE, NULL);
		MmProbeAndLockPages(mdl, KernelMode, IoWriteAccess);
#else
		VirtualProtect(addr, numBytes, PAGE_READWRITE, &oldprotect);
#endif

		if (!memcmp(addr, newCode, numBytes))
			memcpy(addr, oldCode, numBytes);

#ifdef KERNEL
		MmUnlockPages(mdl);
		IoFreeMdl(mdl);
#else
		VirtualProtect(addr, numBytes, oldprotect, &oldprotect);
#endif
...
//bool writeAddr(void *addr, const void *trampoline, void *oldCode, void *newCode)
//{
//	__try
//	{
//#ifdef KERNEL
...
//
//#ifdef KERNEL
//		mdl = IoAllocateMdl(addr, 4, FALSE, FALSE, NULL);
//		MmProbeAndLockPages(mdl, KernelMode, IoWriteAccess);
//#else
//		VirtualProtect(addr, 4, PAGE_READWRITE, &oldprotect);
//#endif
//
//		memcpy(oldCode, addr, 4);
//
//		*(DWORD *)addr = (DWORD)trampoline;
//
...
bool writeJump(void *addr, const void *trampoline, void *oldCode, void *newCode)
{
	__try
	{
#ifdef KERNEL
...
#ifdef KERNEL
		mdl = IoAllocateMdl(addr, 5, FALSE, FALSE, NULL);
		MmProbeAndLockPages(mdl, KernelMode, IoWriteAccess);
#else
		VirtualProtect(addr, 5, PAGE_READWRITE, &oldprotect);
#endif

		if (oldCode)
			memcpy(oldCode, addr, 5);

		*(BYTE *)addr = 0xE9;
		*(DWORD *)((BYTE *)addr + 1) = (DWORD)((BYTE *)trampoline - (BYTE *)addr - 5);

		if (newCode)
			memcpy(newCode, addr, 5);
...
bool hookPre(void *addr, const void *hookProc, void *oldCode, void *newCode_)
{
#ifdef KERNEL
	//todo
#else
	BYTE *newCode = new BYTE[30];
#endif
	BYTE *newCodePos = newCode;

	//pushad
	*newCodePos = 0x60; newCodePos += 1;

	//pushfd
	*newCodePos = 0x9C; newCodePos += 1;

	//push esp
	*newCodePos = 0x54; newCodePos += 1;	

	//call hookproc
	*newCodePos = 0xe8; newCodePos += 1;
	*(DWORD *)newCodePos = (DWORD)((BYTE *)hookProc - newCodePos - 4); newCodePos += 4;

	//popfd
	*newCodePos = 0x9D; newCodePos += 1;

	//popad
	*newCodePos = 0x61; newCodePos += 1;

	if (!makeTramp((BYTE *)addr, newCodePos))
	{
#ifdef KERNEL
#else
		delete[] newCode;
	...
	if (!writeJump(addr, newCode, oldCode, newCode_))
	{
#ifdef KERNEL
#else
		delete[] newCode;
...
bool hookNoTramp(void *addr, const void *hookProc, void *oldCode, void *newCode)
{
	if (!writeJump(addr, hookProc, oldCode, newCode))
		return false;
	return true;
}

bool hook(void *addr, const void *hookproc, char *errormsg, void *oldCode, void *newCode)
{
	BYTE *pos, *tramp, *newproc;

	tramp = (BYTE *)VirtualAlloc(NULL, 10, MEM_COMMIT | MEM_TOP_DOWN, PAGE_EXECUTE_READWRITE);
	if (tramp == NULL)
		return false;

	if (makeTramp((BYTE *)addr, tramp, errormsg) == 0)
	{
		VirtualFree(tramp, 10, MEM_RELEASE);
		return false;
	}

	//hook = (HOOK *)malloc(sizeof(HOOK));
	//hook->newproc = (BYTE *)malloc(30);
	//hook->tramp = tramp;
	//hook->addr = (BYTE *)addr;
	//hook->hookproc = (BYTE *)hookproc;
	//memcpy(hook->code, addr, 5);
	//hook->next = hooks;
	//hooks = hook;

	//pos = newproc = (BYTE *)malloc(30);
	pos = newproc = (BYTE *)VirtualAlloc(NULL, 30, MEM_COMMIT | MEM_TOP_DOWN, PAGE_EXECUTE_READWRITE);
	if (newproc == NULL)
		return false;
	...
	*pos = 0x24; pos += 1;
	...
	//mov dword ptr [esp + 4], epiprint
	*pos = 0xc7; pos += 1;
	*pos = 0x44; pos += 1;
	*pos = 0x24; pos += 1;
	*pos = 0x04; pos += 1;
	*(DWORD *)pos = (DWORD)tramp; pos += 4;

	//jmp hookproc
	*pos = 0xE9; pos += 1;
	*(DWORD *)pos = (DWORD)((BYTE *)hookproc - pos - 4); pos += 4;

	if (!writeJump(addr, newproc, oldCode, newCode))
	{
		VirtualFree(newproc, 30, MEM_RELEASE);
		return false;
	}
...
DWORD cloneFuncAndFixToTrue(void *addr, const void *hookproc, char *errormsg, void *oldCode, void *newCode)
{
	BYTE *pos, *tramp, *newproc;
	...
	*(DWORD*)addr = 0xC340c033;
	...
	return (DWORD)newproc;
}




bool hook_cdecl(void *addr, const void *hookproc, char *errormsg, void *oldCode, void *newCode)
{
	BYTE *pos, *tramp, *newproc;

	//tramp = (BYTE *)malloc(10);
	tramp = (BYTE *)VirtualAlloc(NULL, 10, MEM_COMMIT | MEM_TOP_DOWN, PAGE_EXECUTE_READWRITE);
	if (tramp == NULL)
		return false;
	...
	//pos = newproc = (BYTE *)malloc(36);
	pos = newproc = (BYTE *)VirtualAlloc(NULL, 36, MEM_COMMIT | MEM_TOP_DOWN, PAGE_EXECUTE_READWRITE);
	if (newproc == NULL)
		return false;
